...
public class BlockYukuang extends ElementsXiaoyuMod.ModElement {
	@GameRegistry.ObjectHolder("xiaoyu:yukuang")
	public static final Block block = null;
	public BlockYukuang(ElementsXiaoyuMod instance) {
		super(instance, 7);
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new BlockCustom().setRegistryName("yukuang"));
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("xiaoyu:yukuang", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldXiaoyuworld.DIMID)
			dimensionCriteria = true;
		if (dimID == 0)
			dimensionCriteria = true;
		if (dimID == WorldXiaoyuworld.DIMID)
			...
			(new WorldGenMinable(block.getDefaultState(), 5, new com.google.common.base.Predicate<IBlockState>() {
				public boolean apply(IBlockState blockAt) {
					...
					if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
						blockCriteria = true;
					if (blockAt.getBlock() == BlockXaioyublock.block.getDefaultState().getBlock())
						blockCriteria = true;
